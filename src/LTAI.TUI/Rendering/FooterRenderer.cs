...
public sealed class FooterRenderer
{
        ...
        else
        {
            ...
            if (showWatermark)
            {
                ...
                renders.Add(new Markup(
                    $"{cursor} [{ThemeService.MutedTag}] 输入消息  Enter=发送  Shift+Enter=换行  ↑↓=光标  /开命令  Ctrl+↑↓=历史[/]"));
            }
            else
            {
                ...
                foreach (var (line, idx) in visibleLines.Select((l, i) => (l, i)))
                {
                    ...
                    if (isCursorLine)
                    {
                        ...
                        var col = Math.Min(cursorCol, escaped.Length);
                        var before = escaped[..col];
                        var after = escaped[col..];
                        var cursorChar = cursorBlink ? $"[bold {ThemeService.PrimaryTag}]▌[/]" : " "; 
                        renders.Add(new Markup($"  {ChatRenderer.HighlightCommands(before)}{cursorChar}{ChatRenderer.HighlightCommands(after)}"));
                    }
                    else
                    {
                        renders.Add(new Markup($"  {ChatRenderer.HighlightCommands(line.EscapeMarkup())}"));
                    }
                }
            }
        }
    ...
    private static string BuildStatusLine(string pickerText, string statusText, string? startupMessage)
    {
        ...
        if (!string.IsNullOrEmpty(statusText))
            return $"[{ThemeService.MutedTag}]{statusText.EscapeMarkup()}[/]  [{ThemeService.MutedTag}]{viewName}[/]";
        ...
        if (r > 0)
        {
            ...
            var bal = UsageTracker.BalanceDisplay;
            if (!string.IsNullOrEmpty(bal))
                sb.Append($"余额: {bal.EscapeMarkup()}{sep}");
