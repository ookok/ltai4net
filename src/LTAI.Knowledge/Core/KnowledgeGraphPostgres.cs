// Requires: dotnet add package Npgsql
...
public class KnowledgeGraphPostgres : IDisposable
{
    ...
    private readonly ILogger<KnowledgeGraphPostgres> _logger;
    private readonly NpgsqlConnection _db;
    private readonly string _connectionString;
    private bool _disposed;

    public KnowledgeGraphPostgres(ILogger<KnowledgeGraphPostgres> logger, string? connectionString = null)
    {
        _logger = logger;
    ...
    private void LoadFromPostgres()
    {
        ...
        try
        {
            ...
            while (reader.Read())
            {
                ...
                if (!reader.IsDBNull(2))
                {
                    try
                    {
                        ...
                        var props = System.Text.Json.JsonSerializer.Deserialize<Dictionary<string, object?>>(json);
                        if (props != null)
                        {
                    ...
                    catch (Exception ex) { _logger.LogWarning(ex, "Failed to deserialize entity properties for {Id}", ent.Id); }
                }
            }
            ...
            while (reader2.Read())
            {
                var sid = reader2.GetString(0);
                var tid = reader2.GetString(1);
                var rel = reader2.GetString(2);
            ...
            while (reader3.Read())
            {
                _triplets.Add(new Triplet(
                    reader3.GetString(0), reader3.GetString(1), reader3.GetString(2),
                    reader3.IsDBNull(4) ? "" : reader3.GetString(4),
                    reader3.IsDBNull(3) ? 0.0 : reader3.GetDouble(3)));
            }
        }
    ...
    private void PersistEntity(Entity entity)
    {
        ...
        cmd.Parameters.AddWithValue("@props",
            entity.Properties != null
                ? (object)System.Text.Json.JsonSerializer.Serialize(entity.Properties)
                : DBNull.Value);
        cmd.ExecuteNonQuery();
    }
    ...
    public List<Triplet> SearchTriplets(string query, int limit = 20)
    {
        ...
        try
        {
            ...
            using var cmd = _db.CreateCommand();
            cmd.CommandText = """
                SELECT subject, predicate, object, confidence, source_text
                FROM triplets
    ...
    public List<Entity> SearchEntities(string query, int limit = 20)
    {
        ...
        try
        {
            ...
            cmd.CommandText = """
                ...
                FROM entities
                WHERE label ILIKE @q OR properties::text ILIKE @q
                ORDER BY similarity(label, @query) DESC
                LIMIT @limit
                """;
            cmd.Parameters.AddWithValue("@q", $"%{query}%");
            ...
            while (reader.Read())
            {
                ...
                if (!reader.IsDBNull(2))
                {
                    ...
                    catch (Exception ex) { _logger.LogWarning(ex, "Failed to deserialize entity properties in search"); }
                }
                results.Add(ent);
            }
            return results;
    ...
    public List<string> GetConnectedEntities(string entityId, int maxDepth = 2)
    {
        _lock.EnterReadLock();
        try
        {
    ...
    public KnowledgeGraphPostgres GetSubgraph(List<string> ids)
    {
        _lock.EnterReadLock();
        try
        {
            var sub = new KnowledgeGraphPostgres(_logger, _connectionString);
    ...
    public Dictionary<string, object> GetStats()
    {
        ...
        try
        {
            ...
            return new()
            {
                ...
                ["storage"] = "PostgreSQL",
                ["connection"] = _connectionString.Contains("Host=") || _connectionString.Contains("host=") || _connectionString.Contains("Server=") || _connectionString.Contains("server=")
                    ? _connectionString.Split(';').FirstOrDefault(s => s.StartsWith("Host=", StringComparison.OrdinalIgnoreCase) || s.StartsWith("Server=", StringComparison.OrdinalIgnoreCase)) ?? "(masked)"
                    : "(masked)"
            };
        }
        finally { _lock.ExitReadLock(); }
    ...
    public Dictionary<string, object> GetGraphStats()
    {
        return GetStats();
    }
    ...
    private void LoadFromJsonFile(string path)
    {
        try
        {
            ...
            _logger.LogInformation("KnowledgeGraphPostgres: Imported from JSON, migrated to PostgreSQL");
            try { File.Delete(path); } catch (Exception ex) { _logger.LogWarning(ex, "Failed to delete legacy JSON file"); }
        }
        catch (Exception ex)
        {
    ...
    public static KnowledgeGraphPostgres CreatePostgres(string connectionString)
    {
        var loggerFactory = LoggerFactory.Create(b => b.AddConsole().SetMinimumLevel(LogLevel.Warning));
        var logger = loggerFactory.CreateLogger<KnowledgeGraphPostgres>();
        return new KnowledgeGraphPostgres(logger, connectionString);
    }
